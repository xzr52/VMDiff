...
from piplines.flux_inference_pipline import FluxPipeline #FluxPipeline, \
import torch
...
class StableDiffusion:
    ...
    def __init__(
        ...
        self.te_device_torch =  torch.device("cuda:1")#self.accelerator.device
    ...
    def load_model(self):
        ...
        if self.model_config.is_xl or self.model_config.is_ssd or self.model_config.is_vega:
            ...
            text_encoder_2.to( torch.device("cuda:1"), dtype=dtype)
            flush()
            ...
            text_encoder.to( torch.device("cuda:1"), dtype=dtype)
            ...
            flush()
            text_encoder[0].to( torch.device("cuda:1"))
            text_encoder[0].requires_grad_(False)
            text_encoder[0].eval()
            text_encoder[1].to( torch.device("cuda:1"))
