...
def load_flow_model(
    ...
    # Loading Flux
    print("Init model")
    ckpt_path = "/opt/data/private/image_to_3d/bigmodel/flux/flux1-schnell.safetensors"#configs[name].ckpt_path
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512) -> HFEmbedder:
    sd = load_sft('/opt/data/private/image_to_3d/bigmodel/flux/t5xxl_fp8_e4m3fn_scaled.safetensors', device=str(device))
    sd = optionally_expand_state_dict(T5EncoderModel, sd)
    missing, unexpected = T5EncoderModel.load_state_dict(sd, strict=False, assign=True)
    # max length 64, 128, 256 and 512 should work (if your sequence is short enough)
    return HFEmbedder("google/t5-v1_1-xxl", max_length=max_length, torch_dtype=torch.bfloat16).to(device)
    #return HFEmbedder("/root/.cache/huggingface/hub/models--black-forest-labs--FLUX.1-dev/snapshots/0ef5fff789c832c5c7f4e127f94c8b54bbcced44/text_encoder_2", max_length=max_length, torch_dtype=torch.bfloat16).to(device)
...
def load_ae(name: str, device: str | torch.device = "cuda", hf_download: bool = True) -> AutoEncoder:
    ckpt_path = "/opt/data/private/image_to_3d/bigmodel/flux/ae.safetensors"#configs[name].ae_path
    if (
        ckpt_path is None
        and configs[name].repo_id is not None
