...
class ReduxImageEncoder(nn.Module):
    ...
    siglip_model_name="/opt/data/private/xzr/VMDIFF/siglip-so400m-patch14-384/9fdffc58afc957d1a03a25b10dba0329ab15c2a3"
    def __init__(
        self,
        device,
        redux_dim: int = 1152,
        txt_in_features: int = 4096,
        redux_path: str='/opt/data/private/xzr/VMDIFF/flux1-redux-dev.safetensors', #| None = os.getenv("FLUX_REDUX"),
        ...
        with self.device:
            ...
            self.siglip = SiglipVisionModel.from_pretrained(self.siglip_model_name,local_files_only=True).to(dtype=dtype)
        self.normalize = SiglipImageProcessor.from_pretrained(self.siglip_model_name,local_files_only=True)

    def __call__(self, x: Image.Image) -> torch.Tensor:
        imgs = self.normalize.preprocess(images=[x], do_resize=True, return_tensors="pt", do_convert_rgb=True)#[1, 3, 384, 384]

        _encoded_x = self.siglip(**imgs.to(device=self.device, dtype=self.dtype)).last_hidden_state#[1, 729, 1152]

        projected_x = self.redux_down(nn.functional.silu(self.redux_up(_encoded_x)))#[1, 729, 4096]
