...
def main():
    args = build_args()
    height = args.width
    print(args.device_mode)
    cfg = RunConfig(
        image_folder_path=os.path.dirname(args.img1),
        save_image_folder_path=args.output_dir,
        flux_ckpt_path="black-forest-labs/FLUX.1-Krea-dev",#
        grounding_dino_config="GroundingDino/Ground_DINO_config/GroundingDINO_SwinT_OGC.py",
        grounding_dino_ckpt="ckpts/groundingdino_swint_ogc.pth",
        efficientsam_ckpt="ckpts/efficientsam_s_gpu.jit",
        seed=args.seed,
        width=args.width,
        height=height,
    ...
    # ======= 单对推理入口 =======
    runner.run_one_pair(args.img1, args.img2)
